...
class _StubFoundationTransport(httpx.AsyncBaseTransport):
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json=[
                {"id": "COD", "slug": "codestra", "name": "Codestra", "status": "ACTIVE"},
                {"id": "SMT", "slug": "smith-transport", "name": "Smith Transport", "status": "ACTIVE"},
            ],
        )
...
def authority(monkeypatch):
    ...
    def token(role="platform_admin", scope="platform.tenants.read", subject="operator-1", **overrides):
        current = int(time.time())
        claims = {
            "iss": ISSUER, "aud": AUDIENCE, "azp": "platform-console",
            "sub": subject, "iat": current, "exp": current + 300,
            "jti": str(uuid4()), "scope": scope,
...
async def test_platform_admin_lists_all_tenants(authority, client_factory):
    ...
    # Proves list_tenants requests the narrower foundation.tenant.list scope,
    # not foundation.admin - the token endpoint call is on the stub token
    # manager, so this only asserts the client-side call shape via the path.
