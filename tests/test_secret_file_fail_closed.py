...
def test_rendered_files_replace_defaults_and_never_enter_the_environment(tmp_path):
    ...
    database.write_text("postgresql+asyncpg://runtime:rendered@db.internal/codestra\n", encoding="utf-8")
    ...
    assert settings.database_url == "postgresql+asyncpg://runtime:rendered@db.internal/codestra"
...
@pytest.mark.parametrize("prepare", [
    ...
    lambda p: p.write_text("", encoding="utf-8"),    # empty: rendering failed or lease expired and was scrubbed
    lambda p: p.write_text("   \n", encoding="utf-8"),
])
def test_unavailable_or_empty_rendering_fails_closed_without_plaintext_fallback(tmp_path, prepare):
    path = tmp_path / "database"
    prepare(path)
    settings = make(database_url_file=str(path), database_url="postgresql+asyncpg://fallback@localhost/plaintext")
