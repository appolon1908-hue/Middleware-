...
CREATE TABLE IF NOT EXISTS connector_sdk.connector_installations (
    ...
    FOREIGN KEY (connector_id, current_version)
        REFERENCES connector_sdk.connector_manifests (connector_id, version),
...
CREATE TABLE IF NOT EXISTS connector_sdk.connector_connections (
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, installation_id, external_account_reference),
...
CREATE TABLE IF NOT EXISTS connector_sdk.connector_webhook_endpoints (
    ...
    CHECK (route_path LIKE '/v1/webhooks/%' OR route_path LIKE '/internal/v1/adapters/%'),
...
CREATE INDEX IF NOT EXISTS connector_connections_tenant_state_idx ON connector_sdk.connector_connections (tenant_id, state);
CREATE INDEX IF NOT EXISTS connector_webhook_inbox_pending_idx ON connector_sdk.connector_webhook_inbox (processing_state, received_at) WHERE processing_state IN ('PENDING', 'FAILED');
CREATE INDEX IF NOT EXISTS connector_operations_tenant_state_idx ON connector_sdk.connector_operations (tenant_id, state, created_at);
CREATE INDEX IF NOT EXISTS connector_outbox_available_idx ON connector_sdk.connector_outbox (available_at, created_at) WHERE delivered_at IS NULL;

ALTER TABLE connector_sdk.connector_connections ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_connections FORCE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_webhook_endpoints ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_webhook_endpoints FORCE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_webhook_inbox ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_webhook_inbox FORCE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_operations ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_operations FORCE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_outbox ENABLE ROW LEVEL SECURITY;
ALTER TABLE connector_sdk.connector_outbox FORCE ROW LEVEL SECURITY;

CREATE POLICY connector_connections_tenant_policy ON connector_sdk.connector_connections
...
CREATE POLICY connector_webhook_endpoints_tenant_policy ON connector_sdk.connector_webhook_endpoints
    USING (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid);
CREATE POLICY connector_webhook_inbox_tenant_policy ON connector_sdk.connector_webhook_inbox
    USING (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid);
CREATE POLICY connector_operations_tenant_policy ON connector_sdk.connector_operations
    USING (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid);
CREATE POLICY connector_outbox_tenant_policy ON connector_sdk.connector_outbox
    USING (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid)
    WITH CHECK (tenant_id = NULLIF(current_setting('codestra.tenant_id', true), '')::uuid);
