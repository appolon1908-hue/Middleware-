...
Usage (from a host that can reach the staging edge):
  ...
  CERTIFY_EXPECTED_AUDIENCE=codestra-middleware \\
...
def load_settings(env: dict[str, str], *, static_only: bool, wait_for_expiry: bool) -> Settings:
    ...
    return Settings(
        ...
        base_url=base_url,
        issuer=issuer,
        audience=env.get("CERTIFY_EXPECTED_AUDIENCE", "codestra-middleware"),
    ...
    for route in document.get("contractRoutes", []):
        ...
        if authority.is_file():
            ...
            for row in manifest.get("routes", []):
                if row.get("path") in route.get("paths", []):
                    scope = row.get("scope")
    ...
    for method, template, scope in CANONICAL_ROUTES:
        ...
        if entry:
            ...
            report.add(
                ...
                {"jwt", "post-function", "correlation-id"} <= set(entry.get("plugins", [])),
                str(entry.get("plugins")),
            )
    services = {v["service"] for v in declared.values() if v.get("name", "").startswith("codestra-campaign")}
    report.add(
        phase,
...
def static_keycloak(report: Report, repo: Path | None, digest: str | None, client_ids: dict[str, str]) -> None:
...
def run_static(report: Report, settings: Settings) -> None:
    ...
    static_keycloak(report, settings.keycloak_repo, digest, settings.client_ids)
