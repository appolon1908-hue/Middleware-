...
APPROVED_READ_ONLY_SCRIPT_INVOCATIONS: dict[
    ...
    "appolon1908-hue/Middleware-": {
        ...
        "scripts/apply_repository_governance.py": (
            "1c14c21ae027f293f39b4d3965c39375a53b6dcda71bd977e6f24fab14a5d265",
        ...
        if normalized.startswith("docker/build-push-action@"):
            ...
            if isinstance(tags, str):
                for raw_subject in re.split(r"[\r\n,]+", tags):
                    subject = normalized_image_subject(raw_subject, environment)
...
def attestation_subjects(
    ...
    if isinstance(uses, str) and uses.startswith(
        ...
        if not isinstance(inputs, dict):
            return set()
        subject = normalized_image_subject(inputs.get("subject-name"), environment)
...
def require_reachable_signer_workflow(workflow: str, path: str) -> None:
    ...
    for publication_job in publication_jobs:
        ...
        for publication_index, publication, publication_environment in publications:
            published = publication_subjects(
                publication,
...
"""
    ...
    complete_multi_subject = partial_multi_subject.replace(
        "      - run: cosign attest --yes ghcr.io/example/repository:sha-0123456",
        "      - run: cosign attest --yes ghcr.io/example/repository:sha-0123456\n"
        "      - run: cosign attest --yes ghcr.io/example/secondary:sha-0123456",
        1,
    )
    require_reachable_signer_workflow(
    ...
    conflicting_step_environment = """jobs:
  publish:
    runs-on: ubuntu-latest
    steps:
      - env:
          IMAGE: ghcr.io/example/repository:sha-0123456
        run: docker push "$IMAGE"
      - env:
          IMAGE: ghcr.io/example/other:sha-0123456
        run: cosign attest --yes "$IMAGE"
"""
    try:
        require_reachable_signer_workflow(
            conflicting_step_environment,
    ...
    require_reachable_signer_workflow(
        conflicting_step_environment.replace(
            "          IMAGE: ghcr.io/example/other:sha-0123456",
            "          IMAGE: ghcr.io/example/repository:sha-0123456",
            1,
        ),
