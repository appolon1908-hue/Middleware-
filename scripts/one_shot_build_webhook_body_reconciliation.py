#!/usr/bin/env python3
"""Build and publish the current-main webhook-body reconciliation candidate."""
...
AUTHORITY_SHA = "83d41e37227ca6c852366b649d8d038d07112c7e"
TARGET_BRANCH = "fix/webhook-body-reconciliation-20260903"
...
actual_main = output("git", "rev-parse", "refs/remotes/origin/main")
if actual_main != AUTHORITY_SHA:
    raise SystemExit(
        f"main moved: expected {AUTHORITY_SHA}, actual {actual_main}"
    )
if subprocess.run(
    ["git", "ls-remote", "--exit-code", "origin", f"refs/heads/{TARGET_BRANCH}"],
    stdout=subprocess.DEVNULL,
    stderr=subprocess.DEVNULL,
).returncode == 0:
    raise SystemExit(f"target branch already exists: {TARGET_BRANCH}")
run("git", "checkout", "--detach", AUTHORITY_SHA)

crypto_path = Path(
    "services/connector-runtime/src/codestra_connector_runtime/api/crypto.py"
)
...
ingress_path = Path(
    "services/connector-runtime/src/codestra_connector_runtime/api/"
    "webhook_ingress.py"
)
replace_once(
    ingress_path,
...
method_anchor = "    async def accept(\n"
reconciliation_method = textwrap.dedent(
    '''\
...
if ingress.count(method_anchor) != 1:
    raise SystemExit("webhook accept anchor changed")
ingress = ingress.replace(method_anchor, reconciliation_method + method_anchor, 1)
enabled_anchor = textwrap.dedent(
    '''\
...
if ingress.count(enabled_anchor) != 1:
    raise SystemExit("webhook enable anchor changed")
ingress = ingress.replace(enabled_anchor, enabled_replacement, 1)
start = ingress.index(
    "        reference, body_created = self.body_store.persist_with_status("
)
end = ingress.index("        return 202, {", start)
...
repository_path = Path(
    "services/connector-runtime/src/codestra_connector_runtime/api/repository.py"
)
repository = repository_path.read_text(encoding="utf-8")
repository_anchor = "    def persist_verified_webhook(\n"
repository_method = textwrap.dedent(
    '''\
...
if repository.count(repository_anchor) != 1:
    raise SystemExit("repository webhook persistence anchor changed")
...
app_path = Path(
    "services/connector-runtime/src/codestra_connector_runtime/api/app.py"
)
service_anchor = textwrap.dedent(
    '''\
...
readiness_anchor = textwrap.dedent(
    '''\
            else:
...
test_path = Path(
    "services/connector-runtime/tests/test_webhook_body_reconciliation.py"
)
...
production_docs = Path("docs/production")
production_docs.mkdir(parents=True, exist_ok=True)
(production_docs / "WEBHOOK_BODY_RECONCILIATION_2026-09-03.md").write_text(
    textwrap.dedent(
        '''\
...
run(
    ...
    "-q",
    "services/connector-runtime/tests/test_webhook_body_reconciliation.py",
    "services/connector-runtime/tests/test_api_helpers.py",
    env=test_env,
)
...
paths = [
    "services/connector-runtime/src/codestra_connector_runtime/api/crypto.py",
    "services/connector-runtime/src/codestra_connector_runtime/api/webhook_ingress.py",
    "services/connector-runtime/src/codestra_connector_runtime/api/repository.py",
    "services/connector-runtime/src/codestra_connector_runtime/api/app.py",
    "services/connector-runtime/tests/test_webhook_body_reconciliation.py",
    "docs/production/WEBHOOK_BODY_RECONCILIATION_2026-09-03.md",
]
run("git", "add", "--", *paths)
run("git", "diff", "--cached", "--check")
run("git", "config", "user.name", "codestra-middleware-reconciler")
run(
    "git",
    "config",
    "user.email",
...
run(
    ...
    "commit",
    "-m",
    "fix(webhooks): reconcile encrypted body outcomes durably",
)
target_sha = output("git", "rev-parse", "HEAD")
if output("git", "rev-parse", "HEAD^") != AUTHORITY_SHA:
    raise SystemExit("unexpected remediation parent")
run("git", "push", "origin", f"HEAD:refs/heads/{TARGET_BRANCH}")
remote_sha = output(
    "bash",
    "-c",
    f"git ls-remote origin refs/heads/{TARGET_BRANCH} | awk '{{print $1}}'",
)
if remote_sha != target_sha:
    raise SystemExit(
        f"remote readback mismatch: expected {target_sha}, actual {remote_sha}"
    )
print("WEBHOOK_BODY_RECONCILIATION=PASS")
