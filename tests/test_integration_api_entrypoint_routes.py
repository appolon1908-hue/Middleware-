...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "KeycloakValidator maps insufficient scope and foreign authorized party to "
        "JWTAuthError -> HTTPException(401); RFC 6750 and the Kong scope guard answer 403. "
        "Fix in app/api/v1/integrations.py _authenticate_n8n/_authenticate_odoo."
    ),
)
@pytest.mark.parametrize("label,method,path,params,token", WRONG_SCOPE_CASES, ids=[c[0] for c in WRONG_SCOPE_CASES])
