...
| Production final | `python:3.13.15-slim-bookworm` | `sha256:00faa2debb87529f9f0764e9491d8ba400a3678976616c3bd7cb193745ac20d1` | No package installation or OS mutation during the final stage |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Middleware API | `Dockerfile.runtime` | `runtime` | official Python 3.13.15 slim Bookworm | `sha256:00faa2debb87529f9f0764e9491d8ba400a3678976616c3bd7cb193745ac20d1` | None in final | None added | `requirements-runtime.txt`, exact hashes | `65532:65532` | explicit `/usr/local/bin/python3.13` entrypoint; `-m uvicorn app.main:create_app --factory` | `/health` liveness; `/ready` is separate readiness | build arg | build arg | build arg | PASS locally |
| Outbox/Temporal worker family | `Dockerfile.runtime` | `worker` | official Python 3.13.15 slim Bookworm | same final digest | None in final | None added | `requirements-runtime.txt`, exact hashes | `65532:65532` | explicit `/usr/local/bin/python3.13` entrypoint; default `-m workers.run_outbox`; deployments may select the packaged Temporal module | Process liveness; readiness is worker/dependency telemetry, not an HTTP liveness probe | build arg | build arg | build arg | PASS locally |
| Connector Runtime | `Dockerfile.runtime` | `connector-runtime` | official Python 3.13.15 slim Bookworm | same final digest | None in final | None added | `requirements-connector-runtime.txt`, exact hashes; installed connector SDK and service wheels | `65532:65532` | explicit `/usr/local/bin/python3.13` entrypoint; `-m uvicorn codestra_connector_runtime.main:app` | `/healthz` liveness; `/readyz` readiness | build arg | build arg | build arg | PASS independently |
