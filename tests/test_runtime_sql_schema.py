...
class Catalog:
    ...
    async def fetch(self, query, names):
        ...
        self.statements.append(query)
        return [dict(table_name=name, structure=json.dumps(self.structures[name]))
                for name in names if name in self.structures]


@pytest.fixture
def catalog(monkeypatch):
    structure = {
        'kind': 'r', 'persistence': 'p', 'row_security': False,
        'force_row_security': False,
        'columns': [{'name': 'tenant_id', 'type': 'text', 'not_null': True,
                     'default': None, 'identity': '', 'generated': '', 'collation': 'pg_catalog.default'}],
        'constraints': [{'name': 'key', 'type': 'p', 'definition': 'PRIMARY KEY (tenant_id)',
                         'validated': True, 'deferrable': False, 'deferred': False}],
        'indexes': [{'name': 'key', 'definition': 'CREATE UNIQUE INDEX key ...', 'valid': True}],
        'triggers': [{'name': 'immutable', 'enabled': 'O', 'definition': 'BEFORE UPDATE', 'function': 'RAISE EXCEPTION'}],
        'internal_triggers': [{'enabled': 'O', 'constraint': 'fk'}],
        'policies': [], 'sequences': [{'increment': 1, 'cycle': False}],
    }
...
def test_valid_structure_uses_catalog_only_readonly_snapshot(catalog):
    verify(catalog)
    assert catalog.transaction_options == {'isolation': 'repeatable_read', 'readonly': True}
    assert catalog.statements == ['SET LOCAL search_path = pg_catalog', schema.CATALOG_SQL]
    assert not catalog.in_transaction


@pytest.mark.parametrize('section', [
    'columns', 'constraints', 'indexes', 'triggers', 'internal_triggers', 'sequences',
])
...
@pytest.mark.parametrize(('field', 'value'), [
    ('type', 'integer'), ('not_null', False), ('default', '42'),
    ('identity', 'a'), ('generated', 's'), ('collation', 'public.changed'),
])
...
@pytest.mark.parametrize(('section', 'field', 'value'), [
    ('constraints', 'validated', False), ('constraints', 'definition', 'CHECK (true)'),
    ('constraints', 'deferred', True), ('indexes', 'valid', False),
    ('indexes', 'definition', 'CREATE INDEX nonunique ...'),
    ('triggers', 'enabled', 'D'), ('triggers', 'function', 'RETURN NEW'),
    ('internal_triggers', 'enabled', 'D'), ('sequences', 'increment', -1),
])
...
@pytest.mark.parametrize('change', ['missing_table', 'extra_table', 'bad_hash', 'history', 'version'])
def test_invalid_contract_rejected_before_catalog_access(tmp_path, monkeypatch, change):
    path = tmp_path / 'contract.json'
    document: dict[str, Any] = {'schema_version': 1, 'migration_history_sha256': 'expected',
...
def test_runner_checks_contract_before_opening_database():
    source = (ROOT / 'scripts/migrate_runtime.py').read_text().split('async def main(', 1)[1]
    assert source.index('load_contract(ROOT, history_digest)') < source.index('asyncpg.connect(')
