...
The current Step 3 runtime uses an in-memory Communications read store even when command storage is Postgres-backed. Before production activation, this read model should be backed by durable storage with migration coverage.
