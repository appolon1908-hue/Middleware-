...
| **Path collision** | ⚠️ | This registry's canonical path is `POST /platform/v1/provisioning/requests`. That exact path is already taken by `app/api/v1/platform.py` - a live, tested, migrated feature for a *different* domain (onboarding infrastructure *services* into the platform catalog, not provisioning *people*). This session deliberately mounted the people-provisioning saga at `/platform/v1/agent-provisioning/requests` instead to avoid the collision. **This conflict is unresolved and needs a decision**: rename the service-catalog feature, or accept `agent-provisioning` as the real canonical path for this registry going forward, or something else. Do not silently pick one. |
...
| Capability | Status | Evidence |
|---|---|---|
| Read operations already wired | EXISTS | `app/adapters/odoo/client.py`: `desired_state.read`, `agents.read`, `leads.read`, `campaigns.read`, `traces.read`, `telephony.projections.read`, `telephony.mappings.read`, `reconciliation.runs.read`, `reconciliation.drifts.read`, `sales.lookup`. These are real, live, callable operations Middleware already has against Odoo. |
