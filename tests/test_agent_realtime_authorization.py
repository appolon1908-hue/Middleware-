...
def authority(monkeypatch):
    ...
    def token(tenant_ids=("TEN1",), scope="identity.request", subject="provisioning-service-subject", azp="provisioning-service"):
...
async def _seed_campaign_membership(session_factory, *, employee_id, tenant_id, campaign_id):
    async with session_factory() as session:
        await session.execute(
            text(
                """INSERT INTO agent_provisioning_request
                   ...
                   VALUES (gen_random_uuid(), :request_id, :tenant_id, :employee_id,
                           :email, 'EFFECTIVE', CAST(:campaigns AS jsonb), '{}'::jsonb,
            ...
            ),
            {
                ...
                "campaigns": f'[{{"campaign_id": "{campaign_id}", "role": "agent"}}]',
...
async def _seed_extension_reservation(session_factory, *, employee_id, extension, state="ACTIVE"):
    async with session_factory() as session:
        pool_id = (
            await session.execute(text("SELECT id FROM telephony_extension_pool LIMIT 1"))
        ).scalar_one()
        await session.execute(
            text(
...
async def test_real_campaign_and_extension_membership_is_authorized(authority, client):
    ...
    await _seed_campaign_membership(
        session_factory, employee_id="employee-real-1", tenant_id="TEN1", campaign_id="REAL_CAMPAIGN",
    )
    await _seed_extension_reservation(session_factory, employee_id="employee-real-1", extension=7001)
...
async def test_campaign_not_in_membership_is_rejected(authority, client):
    http_client, session_factory = client
    await _seed_campaign_membership(
        session_factory, employee_id="employee-real-1", tenant_id="TEN1", campaign_id="OTHER_CAMPAIGN",
    )
    await _seed_extension_reservation(session_factory, employee_id="employee-real-1", extension=7001)
    response = await http_client.post(
        "/api/v1/agent/events",
...
async def test_extension_not_owned_by_agent_is_rejected(authority, client):
    ...
    await _seed_extension_reservation(session_factory, employee_id="someone-else", extension=7001)
    response = await http_client.post(
        "/api/v1/agent/events",
        json=envelope(),
        headers={"Authorization": "Bearer " + authority()},
