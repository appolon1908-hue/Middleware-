...
async def get_activity(
    ...
    if source == "agent_provisioning_audit":
        ...
        provisioning_row = (await session.execute(provisioning_detail_stmt)).first()
        ...
        audit_detail_stmt = select(AuditEvent).where(
            AuditEvent.id == UUID(raw_id),
            AuditEvent.redacted_payload["business_unit"].astext == tenant_id,
        )
        audit_row = (await session.execute(audit_detail_stmt)).scalar_one_or_none()
