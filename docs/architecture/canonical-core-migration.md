...
| Topic | Decision | Why |
| --- | --- | --- |
| `SEND_EVENTS` | Single gate. Enabling it needs the JetStream transport rules (`OUTBOX_DISPATCH_ENABLED`/`NATS_DISPATCH_MODE`/activation identity) **and** every broad-event gate with bounded scope. Because `N8N_PRODUCTION_WORKFLOWS_ENABLED` is declared unimplemented by this runtime, JetStream dispatch cannot be enabled by configuration alone until that effect is promoted to `SUPPORTED_EXTERNAL_EFFECTS` by an explicit approval (`tests/test_security.py`). Every deployed profile already sets it to `false`. | Both lineages owned the name with different rules; the union is the only non-weakening merge. |
...
- Promotion (or removal) of `N8N_PRODUCTION_WORKFLOWS_ENABLED` as a supported effect before JetStream dispatch can ever be enabled.
