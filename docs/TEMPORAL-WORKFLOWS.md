...
`workers/run_temporal.py` registers fail-closed activity bindings until the durable command ledger owns their state transitions. Calling an unbound production activity fails with `CapabilityDisabled`; it cannot silently claim success.
...
`tests/integration/test_temporal_workflows.py` runs against Temporal's time-skipping test server and proves activity retry, a one-day durable timer, provisioning compensation, operator-gated dead-letter recovery, and workflow result persistence.
